    ...
    def mutate(root, info, input=None):
        ...
        if user_instance:
            ...
            if input.access_survey is not None:
                user_instance.access_survey = input.access_survey or False
            user_instance.save(commit=False)
            user_instance.name = str(user_instance.first_name) + \
...
class VerifyEmail(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        uuid = force_str(urlsafe_base64_decode(input.uuid))
...
class ResendVerificationEmail(graphene.Mutation):
    ...
    def mutate(root, info, email):
        ...
        if user_instance.is_active == False:
            ...
            return ResendVerificationEmail(user=user_instance,
