...
from common.models import (Company, Diagnostic, Priority, Question,
                           QuestionResponse, Specialization, Tooth, Treatment,AppointmentCode)
from django.shortcuts import get_object_or_404
from graphene_file_upload.scalars import Upload

from .models import (Appointment, AppointmentDetail, AppointmentFile,
                     AppointmentPriority, AppointmentSpecialization,
                     AppointmentSurvey, AppointmentSurveyQuestionResponse,
                     Disease, DiseaseAnswer, Survey, SurveyQuestionResponse,
                     SurveysQuestion,AppointmentLang,AppointmentShortCode)
from .types import (AppointmentDetailType, AppointmentFileType,
                    AppointmentProrityType, AppointmentSpecializationType,
                    AppointmentSurveyQuestionResponseType,
                    AppointmentSurveyType, AppointmentType, DiseaseAnswerType,
                    DiseaseType,AppointmentLangType,AppointmentShortCodeType)
...
class AppointmentDetailsInput(graphene.InputObjectType):
    ...
    # appointment_code = graphene.List(graphene.ID)
    created_by = graphene.ID()
    updated_by = graphene.ID()
...
class CreateAppointmentDetails(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        # for id in input.appointment_code:
        #     appointment_details_instance.appointment_code.add(id)
...
class UpdateAppointmentDetails(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        appointment_details_instance = AppointmentDetail.objects.get(pk=input.id)
...
class UpdateAppointmentPriortiy(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        appointment_priority_instance = AppointmentPriority.objects.get(pk=input.id)
...
class DeleteAppointmentPriority(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        appointment_priority_instance = get_object_or_404(AppointmentPriority, pk=id)
...
class CreateAppointmentSpecialization(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        appointment = get_object_or_404(Appointment, pk=input.appointment)
        specialization = get_object_or_404(Specialization, pk=input.specialization)
        created_by = get_object_or_404(User, pk=input.created_by)
        updated_by = get_object_or_None(User, pk=input.updated_by)
...
class UpdateAppointmentSpecialization(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        appointment_specialization_instance = AppointmentSpecialization.objects.get(pk=input.id)
...
class DeleteAppointmentSpecialization(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        appointment_specialization_instance = get_object_or_404(AppointmentSpecialization, pk=id)
...
class CreateDiseaseAnswer(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        questionResponse = get_object_or_None(QuestionResponse, pk=input.questionResponse)
        created_by = get_object_or_404(User, pk=input.created_by)
        updated_by = get_object_or_None(User, pk=input.updated_by)
...
class DeleteDiseaseAnswer(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        delete_disease_answer_instance = get_object_or_404(DiseaseAnswer, pk=id)
...
class UpdateAppointmentSurvey(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        update_appointment_survey_instance = AppointmentSurvey.objects.get(pk=input.id)
...
class DeleteAppointmentSurvey(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        delete_appointment_survey_instance = get_object_or_404(AppointmentSurvey,id=id)
...
class CreateAppointmentSurveyQuestionResponse(graphene.Mutation):
    ...
    appointmentSurveyQuestionResponse = graphene.Field(AppointmentSurveyQuestionResponseType)

    @staticmethod
    def mutate(root, info, input=None):
        appointment_survey = get_object_or_404(AppointmentSurvey, pk=input.appointment_survey)
        survey_question = get_object_or_404(SurveysQuestion, pk=input.survey_question)
        if input.question_response and input.question_response!="null":
            question_response = get_object_or_404(SurveyQuestionResponse, pk=input.question_response)
...
class UpdateAppointmentSurveyQuestionResponse(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        update_appointment_survey_question_response_instance = AppointmentSurveyQuestionResponse.objects.get(pk=input.id)
...
class DeleteAppointmentSurveyQuestionResponse(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        delete_appointment_survey_question_response_instance = get_object_or_404(AppointmentSurveyQuestionResponse, id=id)
...
class UpdateAppointmentLang(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        lang = get_object_or_404(MultiLanguage, pk=input.lang)
        note = input.note
        update_appointment_lang_instance = AppointmentLang.objects.get(pk=input.id)
...
class DeleteAppointmentLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        delete_appointment_lang_instance = get_object_or_404(AppointmentLang, id=id)
        delete_appointment_lang_instance.delete()
        return None
       
# End Multilanguage

#start AppointmentCode 
...
class CreateAppointmentShortCode(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        appointment = get_object_or_404(Appointment, pk=input.appointment)
        appointment_code = get_object_or_404(AppointmentCode, pk=input.appointment_code)
        created_by = get_object_or_404(User, pk=input.created_by)
        updated_by = get_object_or_None(User, pk=input.updated_by)
...
class UpdateAppointmentShortCode(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        created_by = get_object_or_404(User, pk=input.created_by)
        updated_by = get_object_or_None(User, pk=input.updated_by)
        update_appointment_short_code_instance = AppointmentShortCode.objects.get(pk=input.id)
...
class DeleteAppointmentShortCode(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        delete_appointment_short_code_instance = get_object_or_404(AppointmentShortCode, id=id)
