...
from .models import (Duration, Diagnostic,
                     Treatment, Tooth, Specialization,
                     Company, CompanyUser, LookUp,
                     Question, QuestionResponse,
                     CompanyLang,DiagnosticLang,TreatmentLang,
                     SpecializationLang,QuestionLang,QuestionResponseLang,
                     Qualification,Priority,
                     QualificationLang,PriorityLang,AppointmentCode,
                     )
...
class CreateDiagnostic(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        diagnostic = Diagnostic.objects.filter(name=input.name).first()
        if diagnostic:
...
class CreateTreatment(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        treatment = Treatment.objects.filter(name=input.name).first()
        if treatment:
...
class UpdateCompanyInput(graphene.InputObjectType): #Created By field has been removed
    id = graphene.ID()
    name = graphene.String()
...
class UpdateDiagnosticLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        diagnostic_lang_instance = get_object_or_404(DiagnosticLang,pk=input.id)
        if input.lang:
            lang = MultiLanguage.objects.get(pk=input.lang)
...
class UpdateSpecializationLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        specialization_lang_instance = get_object_or_404(SpecializationLang,pk=input.id)
        if input.lang:
            lang = MultiLanguage.objects.get(pk=input.lang)
...
class DeleteSpecializationLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id=None):
        specialization_lang_instance = get_object_or_404(SpecializationLang,pk=id)
...
class CreateQuestionResponseLang(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        lang = MultiLanguage.objects.get(pk=input.lang)
        question_response = QuestionResponse.objects.get(pk=input.question_response)
...
class UpdateQuestionResponseLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        question_response_lang_instance = get_object_or_404(QuestionResponseLang,pk=input.id)
        if input.lang:
            lang = MultiLanguage.objects.get(pk=input.lang)
...
class DeleteQuestionResponseLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        question_response_lang_instance = get_object_or_404(QuestionResponseLang,pk=id)
...
class UpdateQualificationLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        qualification_lang_instance = get_object_or_404(QualificationLang,pk=input.id)
        if input.lang:
            lang = MultiLanguage.objects.get(pk=input.lang)
...
class DeleteQualificationLang(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, id):
        qualification_lang_instance = get_object_or_404(QualificationLang,pk=id)
...
#End Multi Language Class

#Appointments Code class Start
...
class UpdateAppointmentCode(graphene.Mutation):
    ...
    @staticmethod
    def mutate(root, info, input=None):
        appointment_code_instance = get_object_or_404(AppointmentCode,pk=input.id)
        if input.code_type:
...
class Mutation(graphene.ObjectType):
    ...
    #End Multi language Mutation
    
    #AppointmentCode Mutation start
